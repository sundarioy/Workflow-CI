...
def train_stroke_prediction_model(dataset, target_column='stroke'):
    ...
    # End any existing run to ensure fresh start for autolog
    active_run = mlflow.active_run()
    if active_run:
        print(f"🔄 Ending existing MLflow run: {active_run.info.run_id}")
        mlflow.end_run()
    
    # Configure MLflow autologging BEFORE starting new run
    ...
    # Always create fresh run for optimal autolog performance
    with mlflow.start_run(run_name="Stroke_Prediction_LogisticRegression") as fresh_run:
        print(f"🚀 Started fresh MLflow run: {fresh_run.info.run_id}")
        run_id = fresh_run.info.run_id
        ...
        # Train the model - autolog will capture everything automatically
        ...
        print("🎯 Full model artifacts should be logged by autolog")
